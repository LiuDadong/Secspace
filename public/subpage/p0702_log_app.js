        ...
        $.silentGet('/man/Log/getLog',pd, function (data) {
            if (data.rt == '0000') {
                if(data.logInfo.length==0){
                    str += '<tr><td colspan="7">暂无日志</td></tr>'
                }
                for (var i in data.logInfo) {
                    str += '<tr>'
                        ...
                        + '<td>' + data.logInfo[i].app_name + '</td>'
                        + '<td>' + data.logInfo[i].app_version + '</td>'
                        + '<td>' + (data.logInfo[i].os_type == 'ios' ? 'iOS' : 'Android') + '</td>'
