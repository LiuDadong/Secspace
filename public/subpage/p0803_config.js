...
var om=$('#om_switch').data('om'),
    org_code='';
if(om.selected){
    org_code=om.selected.data.orgCode;
}else{
...
function uploadFormInit() {
    ...
    uploadForm.find('input[name=org_code]').val(org_code);
...
function downloadConfigFiles(ele){
    ...
    $('<form action="/common/config_file/download" method="post">' +  // action请求路径及推送方法
                '<input type="text" name="org_code" value="'+org_code+'"/>' + // 文件路径
