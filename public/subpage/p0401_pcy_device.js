...
function add() {
    ...
    $('input[name=gps1]').prop("checked", true);
...
function getPolicyPostData(act) {
    return {
        ...
        wifi: JSON.stringify({
            ...
            wifi: $('input[name=status]').prop('checked')
                ? $('.wifi.append-box input[type=hidden][name=wifi]').data('arrData')
                : []
        })
    };
}
...
function add_policy() {
    ...
    $.actPost('/man/devpolicy/add_policy', getPolicyPostData(), function (data) {
        if (data.rt == '0000') {
...
function mod_policy() {
    ...
    $.actPost('/man/devpolicy/updatePolicy', postData, function (data) {
        if (data.rt == '0000') {
            multForm.find('.btnBack').click();
        }
    }, '修改', '策略');
