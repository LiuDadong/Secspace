...
$(function() {
    ...
    $('.logmenu').find('li').eq(5).addClass('active');
    getloglist(1,10);
    $("select[name=log_type]").change(function(){
        getloglist(1,10);
    });
        ...
        if (data.rt==0) {
            for(var i in data.logInfo) {
                str += '<tr>'
                    ...
                    + '<td>' + data.logInfo[i].policy_type + '</td>'
                   // + '<td>' + data.logInfo[i].log_type + '</td>'
                    //+ '<td>' + data.logInfo[i].log_type + '</td>'
                    //+ '<td>' + data.logInfo[i].log_type + '</td>'      
...
function impexcel(){   
    ...
    var url = hosturl + 'p/org/exportExcel?sid='+sid+'&category=policyLog';
    downloadFile(url);
}
