        ...
        if (data.rt== 0) {
            for(var i in data.doc) {
                ...
                str += '<tr>'
                    ...
                    + '<td style="display:none;">' + JSON.stringify(data.doc[i].app_list) + '</td>'
...
function checkmap(){
    ...
    tab.find('td span').each(function () {
        if ($(this).hasClass('txt')) {
            tr = $(this).parents("tr");
            dev_id[i] = tr.find('td').eq(11).text()*1;
            i = i+1;
        }     
    }); 
    if(i === 1){        
...
function updatescreenpw(dev_id){
    ...
    var passwd_type, min_len, checkpwd='', warningtext = '';
    var postData = {
        dev_id: dev_id,
        id: id
        };
...
function sendcmds(cmd){
    var dev_id = [], i = 0, tr, id, passwd_type, checkpwd='', warningtext = '', min_len;
    var cmd = cmd;
    var tab = $('.devicetable table');
    tab.find('td span').each(function () {
        if ($(this).hasClass('txt')) {
