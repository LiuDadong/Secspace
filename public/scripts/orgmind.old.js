...
; (function ($w) {
    ...
    var om = function (use, opts) {
        ...
        merge(def_jmopt, def_omopt);
        merge(def_jmopt, opts);
        if (!def_jmopt.container) {
            logger.error('the opts.container should not be null or empty.');
            return;
        }
        if (!def_jmopt.om_btns) {
            logger.error('the opts.om_btns should not be null or empty.');
            return;
        }
        this.jm = new jsMind(def_jmopt);
        this.use = use;
        this.omopt = def_jmopt;
        this.inited = false;
        this.init();
        $('#'+def_jmopt.container).addClass('jsmind').data('jm',this.jm);
        return this.jm;
    };
