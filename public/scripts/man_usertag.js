...
$(function() {
    ...
    $('.usermenu').find('li').eq(3).addClass('active');
    // 标签列表
    getTagList(1,10);
});
// app标签列表
...
function tag_update(id){
    ...
    if (postData.name == "") {
        ...
        $.post('/man/tag/updateTag', postData, function(data) {
            if (data.rt == 0) {
...
function tag_add(){
    ...
    if (postData.name == "") {
        ...
        $.post('/man/tag/addTag', postData, function(data) {
            if (data.rt == 0) {
                alertOff();
                warningOpen('操作成功！','primary','fa-check');
                getTagList(currentpage,10); 
...
function tag_delete() {
    ...
    if(tag_ids.length > 0){
        ...
        $.post('/man/tag/delTag', postData, function(data) {
            if (data.rt == 0) {               
                getTagList(1,10);  
                alertOff(); 
                warningOpen('操作成功！','primary','fa-check'); 
